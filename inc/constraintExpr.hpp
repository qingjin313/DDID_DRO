...
public:
    ...
    inline void addConst(const double cst) {rhs -= cst; return;}
    inline ConstraintExpression projectToW(const std::vector<int> indices, int beginW) const{
        ConstraintExpression N;
        N.sense = sense;
        N.rhs = rhs;
        N.name = name;
        N.varIndices = varIndices;
        N.varCoeffs = varCoeffs;
        N.paramIndices = paramIndices;
        N.paramCoeffs = paramCoeffs;
        ...
        for(auto &ind : indices){
            const int Pos = std::find(N.varIndices.begin(), N.varIndices.end(), ind) - N.varIndices.begin();
            N.varIndices.erase(N.varIndices.begin() + Pos);
            N.varCoeffs.erase(N.varCoeffs.begin() + Pos);
        }
        
        indices_t newInd;
        for(auto &ind : N.varIndices){
            newInd.emplace_back(ind - beginW + 1);
        }
        N.varIndices = newInd;
    ...
    inline std::vector<int> getVarIndices() const {return varIndices;}
    inline std::vector<double> getVarCoeffs() const {return varCoeffs;}
                                    ...
                                    std::vector<double>& rmatval) const
    {
         ...
         * and return that constraint deterministically
         */
        ...
        for (unsigned i = 0; i < varIndices.size(); ++i)
            if(varCoeffs[i] != 0.0){
                nzcnt += 1;
                rmatind.emplace_back(varIndices[i]);
                rmatval.emplace_back(varCoeffs[i]);
        }
    }
    ...
    inline ConstraintExpression mapParamK(const unsigned int k, int numParam) const {
        ConstraintExpression N;
        ...
        indices_t paramIndicesNew;
        pairIndices_t bilinearIndicesNew;
        
        for(int ind : paramIndices){
            assert(ind <= numParam);
            paramIndicesNew.emplace_back(ind + (k+1)*numParam);
        }
        
        for(auto ind2 : bilinearIndices){
            assert(ind2.second <= numParam);
            bilinearIndicesNew.emplace_back(std::make_pair(ind2.first, ind2.second + (k+1)*numParam));
        }
        
        N.paramIndices = paramIndicesNew;
        N.bilinearIndices = bilinearIndicesNew;
...
public:
	                                    ...
	                                    const bool normalized = 0) const
	{
		...
		if(donotSolveLP) {
            ...
            stat = CPXXchgbds(env, lp, 1, &ind, &ub, &ubVal);
            if (stat) {std::cerr << "Failed to change objective inside evaluation of K-adaptable expression. \n"; exit(-1);}
            
//            std::vector<double> lbget(5, 0);
//            assert(!CPXXgetlb(env, lp, &lbget[0], 0, 4));
			...
			for(const auto& exp: Expr) {
				...
				stat = TauConstr.addToCplex(env, lp);
